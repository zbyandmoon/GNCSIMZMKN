...
void build_preparations(
    ...
    for (std::size_t ordinal = 0U;
        ...
        if (entry_is<yyz::AerodynamicTablePreparationCall>(
                image, preparation->prepare_entry_handle)) {
            if (occurrence->asset_bindings.size() != 1U) {
                throw std::runtime_error(
                    "aerodynamic occurrence lacks one asset binding");
            }
            auto table = make_aerodynamic_fixture_asset(
                occurrence->asset_bindings.front().asset_id);
            if (table.asset_schema_id !=
                occurrence->asset_bindings.front().asset_schema_id) {
                throw std::runtime_error(
                    "aerodynamic asset schema binding mismatch");
            }
            auto definition = require_outcome(
...
[[nodiscard]] yyz::AerodynamicTableAsset baseline_aerodynamic_asset() {
...
[[nodiscard]] yyz::AerodynamicTableAsset canonical_aerodynamic_asset() {
    const auto baseline = baseline_aerodynamic_asset();
    yyz::AerodynamicTableAsset table;
    table.asset_schema_id = baseline.asset_schema_id;
    table.asset_id = std::string(kCanonicalAerodynamicAssetId);
    table.mach_axis = {0.2, 0.6, 0.8};
    table.alpha_axis_radians = {
        -3.14159265358979323846, -0.1, 0.1,
        3.14159265358979323846};
    table.beta_axis_radians = {
        -1.57079632679489661923, -0.05, 0.05,
        1.57079632679489661923};

    // Continue the one analytic multiaffine relation defined by the eight
    // frozen corners. Old corner values are copied exactly; every outer node
    // is generated by deterministic multilinear continuation.
    for (const double mach : table.mach_axis) {
        const double mach_weight = (mach - 0.2) / 0.4;
        for (const double alpha : table.alpha_axis_radians) {
            const double alpha_weight = (alpha + 0.1) / 0.2;
            for (const double beta : table.beta_axis_radians) {
                const double beta_weight = (beta + 0.05) / 0.1;
                std::array<double, 6U> continued{};
                const bool old_corner =
                    (mach == 0.2 || mach == 0.6) &&
                    (alpha == -0.1 || alpha == 0.1) &&
                    (beta == -0.05 || beta == 0.05);
                if (old_corner) {
                    const std::size_t mach_corner = mach == 0.2 ? 0U : 1U;
                    const std::size_t alpha_corner =
                        alpha == -0.1 ? 0U : 1U;
                    const std::size_t beta_corner =
                        beta == -0.05 ? 0U : 1U;
                    continued = baseline
                        .coefficient_rows_ca_cy_cn_cl_cm_cn[
                            mach_corner * 4U + alpha_corner * 2U +
                            beta_corner];
                } else {
                    for (std::size_t mach_corner = 0U;
                         mach_corner < 2U; ++mach_corner) {
                        const double mach_factor = mach_corner == 0U
                            ? 1.0 - mach_weight
                            : mach_weight;
                        for (std::size_t alpha_corner = 0U;
                             alpha_corner < 2U; ++alpha_corner) {
                            const double alpha_factor = alpha_corner == 0U
                                ? 1.0 - alpha_weight
                                : alpha_weight;
                            for (std::size_t beta_corner = 0U;
                                 beta_corner < 2U; ++beta_corner) {
                                const double beta_factor = beta_corner == 0U
                                    ? 1.0 - beta_weight
                                    : beta_weight;
                                const auto& corner = baseline
                                    .coefficient_rows_ca_cy_cn_cl_cm_cn[
                                        mach_corner * 4U +
                                        alpha_corner * 2U + beta_corner];
                                const double factor = mach_factor *
                                    alpha_factor * beta_factor;
                                for (std::size_t coefficient = 0U;
                                     coefficient < continued.size();
                                     ++coefficient) {
                                    continued[coefficient] +=
                                        factor * corner[coefficient];
                                }
                            }
                        }
                    }
                }
                table.coefficient_rows_ca_cy_cn_cl_cm_cn.push_back(
                    continued);
            }
        }
    }
...
yyz::AerodynamicTableAsset
make_aerodynamic_fixture_asset(std::string_view asset_id) {
    if (asset_id == kBaselineAerodynamicAssetId) {
        return baseline_aerodynamic_asset();
    }
    if (asset_id == kCanonicalAerodynamicAssetId) {
        return canonical_aerodynamic_asset();
    }
    throw std::runtime_error("unknown aerodynamic fixture asset: " +
                             std::string(asset_id));
}
    ...
    install_invocation<yyz::ControlledRigidRuntimeCell>(
        ...
        [options, trace, probe, boundary_factory,
         held_writer_token = closure_invocation.result_writer_token_handle](
            ...
            if (!probe->aerodynamic_envelope_initialized) {
                probe->aerodynamic_envelope_initialized = true;
                probe->opening_airspeed = probe->airspeed;
                probe->opening_alpha = probe->alpha;
                probe->opening_beta = probe->beta;
                probe->opening_dynamic_pressure = probe->dynamic_pressure;
                probe->opening_mach = probe->mach;
                probe->opening_aerodynamic_coefficients =
                    probe->aerodynamic_coefficients;
                probe->minimum_mach = probe->mach;
                probe->maximum_mach = probe->mach;
                probe->minimum_alpha = probe->alpha;
                probe->maximum_alpha = probe->alpha;
                probe->minimum_beta = probe->beta;
                probe->maximum_beta = probe->beta;
            } else {
                probe->minimum_mach =
                    std::min(probe->minimum_mach, probe->mach);
                probe->maximum_mach =
                    std::max(probe->maximum_mach, probe->mach);
                probe->minimum_alpha =
                    std::min(probe->minimum_alpha, probe->alpha);
                probe->maximum_alpha =
                    std::max(probe->maximum_alpha, probe->alpha);
                probe->minimum_beta =
                    std::min(probe->minimum_beta, probe->beta);
                probe->maximum_beta =
                    std::max(probe->maximum_beta, probe->beta);
            }
            probe->contexts.push_back(context_probe(
        ...
        std::make_shared<FixedIntegration>(
            ...
            [options, trace, step, integration_factory,
             ...
             fixed_step_seconds, finite_check](
                ...
                found->held_pitch_moment_newton_meters =
                    held->moment_total_about_center_of_mass.value(1);
