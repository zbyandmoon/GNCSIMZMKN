...
[[nodiscard]] CompleteStaticCompositionSource make_source(
    ...
    return gnc::tests::ref_yyz::make_complete_source(package);
}
...
void verify_complete_ref_graph() {
    ...
    const auto disposition = [](
        ...
        return gnc::contracts::transaction_slot_disposition(branch,
                                                             slot_handle);
    };
    require(std::all_of(
                terminal_only_handles.begin(), terminal_only_handles.end(),
                [&](const auto handle) {
                    return disposition(continue_branch, handle) ==
                               gnc::contracts::TransactionSlotDisposition::
                                   Discard &&
                           disposition(terminal_branch, handle) ==
                               gnc::contracts::TransactionSlotDisposition::
                                   PublishAndSealOutput &&
                           disposition(failure_branch, handle) ==
                               gnc::contracts::TransactionSlotDisposition::
                                   Discard;
                }),
            "terminal result lifetime is not derived solely from transaction branches");
    require(std::all_of(
                ...
                std::all_of(
                    transaction.held_slot_handles.begin(),
                    transaction.held_slot_handles.end(),
                    [&](const auto handle) {
                        return disposition(continue_branch, handle) ==
                                   gnc::contracts::
                                       TransactionSlotDisposition::RetainHeld &&
                               disposition(terminal_branch, handle) ==
                                   gnc::contracts::
                                       TransactionSlotDisposition::Discard &&
                               disposition(failure_branch, handle) ==
                                   gnc::contracts::
                                       TransactionSlotDisposition::Discard;
                    }) &&
                std::all_of(
                    cycle_output_handles.begin(), cycle_output_handles.end(),
                    [&](const auto handle) {
                        return disposition(continue_branch, handle) ==
                                   gnc::contracts::TransactionSlotDisposition::
                                       PublishAndSealOutput &&
            ...
            "candidate, held, or ordinary CycleFrame transaction lifetime changed");
