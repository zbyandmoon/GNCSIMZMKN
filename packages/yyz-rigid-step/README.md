...
two-entity causal qualification 提供两个独立 Vehicle entity、A 的 sampled formal output、B-owned previous-committed link 和 B consumer。package 只贡献 typed definitions、entries 与 codecs；cross-entity selector、multi-scope transaction union、reader/candidate/writer authority 由 Compiler 冻结。A/link/B 三个 owner 的 candidate 在同一个 StepTransaction 提交，任一 candidate 失败会整体回滚。

inactive-child qualification 把 relationship、parent、child 和 downstream consumer 全部预声明到 Source/Image。child 在 Session 初始化时已经 materialize，初始 activity=false；typed activation command 经 relationship reducer、parent mapper 和 child mapper 形成 exact two-hop event chain。三个 owner replacement、receipt、两份 event、activity flip 与 topology revision 在一次 no-fail commit 中发布，child 从下一 Publish Region 开始按冻结 schedule 执行。Kernel 只消费数字 entity/activation/gate handles，不识别 package 或 entity 名称。
...
`Canonical00AInitialMappingQuery` 把精确 author facts 映射到 `frame.yyz.00a.launch-local-enu@1`：ENU position `[0,0,1000] m`、velocity `[220,0,0] m/s`、body forward/right/down 对应 East/South/Down，初始 `q_I_B` 的 double-precision representative 为 `[0,1,3.0615158845559431e-17,0]`，angular rate 为零。query 只接受纬经 `31.2304/121.4737` 的 launch anchor，并同时验证 duration、mass、command 与整数 cadence；Earth rotation、curvature、ellipsoid conversion 和 changing tangent plane 都在适用域外。冻结旧气动资产时的 domain-locked Image fingerprint 为 `e981118b136b3e6872da40a00c296153b9ad26e144c7882341b64b30b219574c`，opening Mach `0.6176470588235294` 会对 `[0.2,0.6]` 闭区间 fail closed。source-selected synthetic successor 保留旧域结果并把真实有限包线固定到 Mach `[0.2,0.8]`、alpha `[-pi,pi]`、beta `[-pi/2,pi/2]`；完成 3000 intervals 的 canonical Image fingerprint 为 `65e00515234efcabfae14399e9fe6838d65248af2d9a598f50b09a3470131078`。旧 interval-1 fingerprint 保持 `7d1fbe1fa09ca555420ed3cc14a05d1f2994c6501b17cd3991355520fc8e6f14`，独立 Decimal oracle 与 difference report 保持 `unresolved_count=0` 和 `abstract_engineering_target_conformance`。
...
ctest --preset dev -R '^r3\.kernel-stuck-actuator\.probe$'
ctest --preset dev -R '^r3\.kernel-two-entity-causal\.probe$'
ctest --preset dev -R '^r3\.kernel-inactive-child-activation\.probe$'
```
