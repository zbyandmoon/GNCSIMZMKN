...
这是 GNCZMKN 大型目标架构的 greenfield 实现仓库。仓库版本仍为 `0.0.0-bootstrap`；R0 科学基线、R1 独立模型生态与 R2 静态编译已经闭合，R3 已形成 caller-bound InitializationCommit、两次 Continue、一次 Terminal、Completed-run ResetCommit、plan-derived cancellation、稳定历史 RunOutcome、显式 dispose，以及 route-free REF-YYZ process-local checkpoint/branch restore 串联的 formal run，并补充 qualification-only ModeOwner 的 command/cutoff/receipt/event 原子事务切片。
...
- R2 Image 保存 exact package/build lock、按 storage class 划分的确定性 extent、slot size/alignment/offset/codec/writer/reader、两个 state codec、Query `CallerLocal` route、Closure `HeldInterval` route、fixed-step RK4 policy、完整 `TransactionPlan` 分支语义、reset lifecycle capability、plan-derived cancellation safe points、preparation lifecycle，以及七个 typed `RuntimeCellFactory` 的数字依赖闭包。分支处置统一从 `TransactionPlan` 派生；cancellation policy 进入既有 Image fingerprint。
...
- `Completed` Session 接受携带新 Session-local unique RunId 与 exact RunBinding 的 reset。Kernel 在任何 staging 前确认七个 Runtime Cell 都声明 `Resettable`；缺失 capability 时保留旧 run 全部证据并形成 `Reset + run_start_committed=false + Failed + Unknown` attempt。成功的 `ResetCommit` 令 sequence 和 epoch 各增加一、tick 回到 Image initial tick，重建 mutable state，并清空 history、seal、terminal result、journal 与本 run step count；Image、provider、PreparedModel 和 Runtime Cell 保持复用。InitializationCommit/ResetCommit 前失败的 finalization 为 `NotStarted`，run-start commit 后执行失败与正常完成为 `Succeeded`。
- `dispose()` 允许 `Created`、`Completed`、`Cancelled` 与 `Failed` 进入 `Disposed`，逆序释放 PreparedModel、Runtime Cell、state/candidate/frame storage、history 与 seal，且清理只执行一次。释放后 materialized counts 为零，Image、最后 committed RunId 和冻结 outcome 继续可查询；析构不重复清理。`Initialized` 拒绝 dispose，活动 run 不会被隐式截断。
- route-free REF-YYZ 可在 `Initialized` committed boundary 发布 immutable process-local checkpoint。checkpoint 深拷贝 committed state、evaluator history 与当前 sealed boundary，并保存 exact Image/RunBinding、parent run identity、epoch/tick/step count 和 continuation frame coordinates。新的 `Created` Session 以新 child RunId、同一 Image 和匹配的 typed implementation 完成普通 preparation/Runtime Cell 物化，随后用一次 `RestoreCommit` 发布私有 state/history/seal 和 lineage；child 从 `run_sequence=0`、checkpoint boundary 继续执行。源 Session 继续运行、reset、dispose 和析构均不影响 checkpoint，同一 checkpoint 可恢复多份互相隔离的分支。
...
最后四个探针通过 fixture adapter 恢复 Image 中的 exact typed entries。Session 初始化只沿数字 handle 建立三个 prepared artifact、七个 Runtime Cell 和两组 committed/candidate state object；frame value 延迟到首次写入时构造，并在 frame 关闭时逆序销毁。`IntegrationHeld` 值始终留在当前 transaction 的 CycleFrame，RK4 与 candidate producer 完成消费后随 frame 一并销毁，不进入 committed output、history、terminal result 或 checkpoint。revision、handle 集合、extent membership、bounds/alignment/overlap、state/lifecycle、cancellation policy、command/event route、materializer identity、精确 preparation 依赖与 type witness 均在 placement 前验证。测试 adapter 提供确定性并发 rendezvous，生产 Kernel 不暴露测试 callback；直接用例覆盖首步前、boundary 间、candidate 间、最终预检、commit 后、Terminal/失败优先、幂等、dispose 与双 Session 取消隔离。ModeOwner qualification 另覆盖 admission/capacity/idempotency、cutoff 前后、Applied/Rejected/Deferred、未知 decision、supersession/expiration/termination、same-tick event ordering、故障分类与重试、queue 物理压缩、pre/postcommit cancellation、reset、checkpoint/restore capability 拒绝和双 Session control-store 隔离。完整 REF-YYZ 运行的三次 step 仍形成三个 committed history samples、tick 0/1/2 ObservationSeal 和 tick 2 terminal result seal；额外丢弃的 environment/aero query 将调用数从各 3 次增加到各 9 次。package-aware qualification snapshot 逐项读取正式 seal 中的具体 C++ payload、SampleContext 和 quality，最终状态、termination 与结果逐字段相同。checkpoint qualification 从 epoch 1/tick 1 继续父分支与两个 child 分支，逐项比较 committed state、concrete history samples、typed seals、terminal decision/result 和 oracle；clone、identity、layout/codec/type/invariant、barrier、allocation 与 final-precheck 故障均无部分发布。reset qualification 在同一 Session 内完成 sequence 0、1、2 三条 run，epoch 依次到达 `3 → 4 → 7 → 8 → 11`；三条 run 的 scientific state、sealed concrete payload 与 terminal mission result 位级一致，frame generation 保持单调，sequence 0/1 outcome 指针跨后续 reset 稳定。request、capability、initial rebuild、clone、replace、validation、outcome/history allocation 和 final precheck 故障均保留旧 committed evidence；Created/Completed/Cancelled/initialization Failed/execution Failed dispose 覆盖一次性清理。

YYZ package 还贡献 `AltitudePitchGuidance` 的 stateless `SampledTransform` descriptor：`vehicle.process` placement、每个 committed boundary 的 `process` phase、`BoundaryEvaluation`、current-cycle sampled input/output、zero-order hold、`Instantiate | Resettable | Dispose` lifecycle 和 exact guidance kernel identity。该 schedule 只描述当前 R1 两区间产品 identity，不覆盖 00A/Reference A 的目标多速率 profile；该窄 descriptor 也不开放 state-schema 字段或重新解释现有混合 output。它的 canonical config 可确定性重建 definition，Catalog 对 form/profile/recipe/obligation/schedule/port/lifecycle 组合做封闭校验：
