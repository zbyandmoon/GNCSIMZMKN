...
这是 GNCZMKN 大型目标架构的 greenfield 实现仓库。仓库版本仍为 `0.0.0-bootstrap`；R0 科学基线、R1 独立模型生态与 R2 静态编译已经闭合，R3 已形成 caller-bound InitializationCommit、两次 Continue、一次 Terminal、空 finalization 与 immutable RunOutcome 串联的 REF-YYZ formal run 纵向切片。
...
- 暂缓开展：完整周期 scheduler、event/cancellation、CommandLedger/queues 与 application receipt、通用多 transaction/多 integration scope 执行、reset/checkpoint/restore、完整 RunResource hooks、完整 DiagnosticRecord/PolicyDecision/renderer/store/artifact 链、YAML/INI、多端 adapter、runtime registry、serializer、StateFragment，以及 R4 Field/Artifact/Dataset/sink 与 R4～R8 其余能力。
- 旧 GNCZMKN 只作为只读行为与科学参照，不进入任何生产 target、include path 或运行依赖。

阶段顺序已经固定：R1 以 Definition、PreparedModel 和 Kernel 的无 Session 独立求值闭合；R2 实现 MissionSource 到已证明、已链接 ExecutionPlan 的静态编译；R3 已完成 formal REF-YYZ run、Session-local store/frame/IntegrationScope 与窄 in-process outcome 主线，后续补齐通用调度、reset/restore/RunResource 和完整运行诊断能力。
