...
- qualification-only ModeOwner 通过可选 Compiler lowering 冻结一条数字 command route、一个 transaction、`InstantPatch` owner candidate 与同 transaction later-phase event delivery。`submit_command()` 只在 `Initialized` committed boundary 由单一执行 owner 调用；per-Session ledger/queue 形成 transaction-start cutoff，`Applied` 的完整 state replacement、application receipt、typed event 与 consumer output 随 ModelCommit 同时可见。reducer/consumer/ObservationSeal invocation failure 允许在同一 committed boundary 重试；普通 projection/boundary/history/integration/scientific candidate 与 final precommit failure 冻结 Failed RunOutcome。已消费 queue 项在提交前压缩，未知 reducer decision、第二条 route、跨 transaction consumer 与未知 transaction 均 fail closed。reset 清空本 run control stores，失败 reset 保留旧 evidence，两份 Session 完全隔离。
- `r3.kernel-two-entity-causal.probe` 冻结两个 Vehicle entity、一个显式跨实体 selector 和一个 multi-scope transaction。A 的 current-cycle formal output 经 typed binding 影响 B，B-owned link 同时保留 previous-committed provenance；A/link/B 三个 owner 仅由各自 candidate writer 更新，并在同一个 no-fail ModelCommit 原子发布。coherently rehashed plan/Image 负例、失败归因、rollback、隔离和确定性均有直接覆盖。
- `r3.kernel-inactive-child-activation.probe` 在 Image revision 5 中预声明并初始化全部 child Runtime Cell/state/slot/resource，同时以 activity gate 阻止 opening 和 activation-tick invocation/output。typed command 经 relationship reducer、parent mapper、child mapper 形成 exact two-hop chain；三个 `InstantPatch` owner、receipt、两份 event、child activity 与 topology revision 在同一 StepTransaction 原子提交，child 从下一 Publish Region 按冻结 cadence 执行。retry、fatal rollback、reset 与 unsupported checkpoint/restore 均 fail closed。
- public runtime failure path 统一覆盖 `create_session`、initialize/reset/checkpoint/restore、`execute_step`、`run_to_terminal`、`submit_command` 与 `dispose`：稳定 code/stage/operation、source kind/field/numeric conformance handle、typed subject/reference、cause、validity、disposition、RunId presence 和 tick/epoch context 均由 outcome 携带。cancellation 保持带稳定 reason 的正常 control outcome，不生成 `RuntimeDiagnostic`。
- 暂缓开展：canonical 00A 的真实飞机气动、地球/大气变化、稳定性/操稳/认证口径和独立 3000-step 轨迹收敛证明，first-order hold/interpolation/extrapolation、异步 clock domain、生产 command consumer、通用多 route/多 transaction scheduler 与 event bus、command/event checkpoint participation、pending-command branch identity、active-run truncating reset、完整 RunResource hooks、durable checkpoint serialization/Artifact 与 compatibility migration、可配置 PolicyDecision、diagnostic renderer/store/durable artifact、YAML/INI、多端 adapter、runtime registry、通用 StateFragment 抽象，以及 R4 Field/Artifact/Dataset/sink 与 R4～R8 其余能力。
- 旧 GNCZMKN 只作为只读行为与科学参照，不进入任何生产 target、include path 或运行依赖。

阶段顺序已经固定：R1 以 Definition、PreparedModel 和 Kernel 的无 Session 独立求值闭合；R2 实现 MissionSource 到已证明、已链接 ExecutionPlan 的静态编译；R3 已完成 formal REF-YYZ run、Completed-run reset、plan-derived cancellation、显式 dispose、route-free process-local checkpoint/branch restore、Session-local store/frame/IntegrationScope、公共 in-process failure context、qualification command/event 原子事务、真实 YYZ edge 的双速率 held-output qualification、00A target-rate 产品运行、canonical 00A mapping/source/Image 与三类 transaction pressure fixture。`R3-FIX-001` 和 `R3-DIA-001` 的技术证据已经闭合并保持 `review`，仓库所有者尚未接受；G4/G5/G6、M1 和扩展科学口径仍由仓库所有者决定。
...
`hash_canonical_mission_ir` 使用 `gnc.canonical-mission-ir.semantic-bytes@2` 的显式 tagged/length-prefixed big-endian encoding 与 SHA-256。source URI/path、输入顺序和 plan id 被排除；C++ 与 Python reference 继续固定 YYZ qualification vector `b29dc67f2a9e0bb36cb18a5e54a8c4830bdb0cae718fbf856646ba903892511b`。RuntimeComponent 仍不会进入该 API。完整图改用 additive `gnc.canonical-mission-ir.semantic-bytes@3`：source semantics覆盖 model/config/asset/port/state/obligation/schedule/temporal/invocation composition，package entry identity、recipe、workspace、state layout、build fingerprint、函数地址与 source location均排除。Optional occurrence/binding/observation extension 使用独立 conditional domain；只有 extension 存在时才编码 source-authored schedule、temporal 与 observation cadence facts。Descriptor revision 6 与基础 Image revision 3 保存 exact implementation、allocation、access、lifecycle 与 cancellation facts；two-entity selector/scope union 使用 conditional revision 4，predeclared inactive-child activation 使用 conditional revision 5。optional command/event、held-output、entity 与 activation extension 分别把自身运行字段纳入 descriptor/Image fingerprint，未选择扩展的 REF-YYZ hash 保持原值。R3 已提供 Session-local committed/candidate/runtime/held-output stores、有界 CycleFrame、transaction-local `IntegrationHeld`、受限 IntegrationScope view、Image-backed fixed-step RK4、multi-owner candidate 原子提交、bounded rolling committed history、ObservationSeal、terminal-window/runwide result seal、caller-bound RunId/RunBinding、InitializationCommit、Completed-run ResetCommit、plan-derived cancellation、immutable RunOutcome、explicit dispose、route-free process-local checkpoint/branch restore、qualification command/event transaction、scheduled stuck-actuator、two-entity causal transaction、predeclared inactive-child activation、真实 sampled edge 的多速率 held injection 和 00A target-rate 产品链。仓库所有者已接受 `aae74f3` 并将 `R3-YYZ-001` 关闭为 `done`；`R3-FIX-001` 与 `R3-DIA-001` 的 technical acceptance evidence 已闭合，两项继续保持 `review` 等待仓库所有者接受。syntax-neutral `SourceTree`/`SourceMap`、通用 `PreparedModelKey`/共享 cache、通用多 transaction scheduler、command/event checkpoint、pending-command branch identity、active-run truncating reset、完整 RunResource hooks、durable checkpoint/diagnostic/artifact、可配置 policy 与 compatibility migration 仍待后续任务；asset proof仍只证明 source-selected identity preservation。G4/G5/G6 尚未由 owner 签署，R4 及后续阶段保持锁定。
