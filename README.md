...
这是 GNCZMKN 大型目标架构的 greenfield 实现仓库。仓库版本仍为 `0.0.0-bootstrap`；R0 科学基线、R1 独立模型生态与 R2 静态编译已经闭合，R3 已形成 caller-bound InitializationCommit、两次 Continue、一次 Terminal、Completed-run ResetCommit、plan-derived cancellation、稳定历史 RunOutcome 与显式 dispose 串联的 REF-YYZ formal run，并补充 qualification-only ModeOwner 的 command/cutoff/receipt/event 原子事务切片。
...
- qualification-only ModeOwner 通过可选 Compiler lowering 冻结数字 command route、`InstantPatch` owner candidate 与 later-phase event delivery。`submit_command()` 只在 `Initialized` committed boundary 由单一执行 owner 调用；per-Session ledger/queue 形成 transaction-start cutoff，`Applied` 的完整 state replacement、application receipt、typed event 与 consumer output 随 ModelCommit 同时可见。失败与 precommit cancellation 回滚 queue maintenance、receipt、event 和 state staging；Terminal 只结算 Expired/Superseded/Terminated maintenance。reset 清空本 run control stores，失败 reset 保留旧 evidence，两份 Session 完全隔离。
- 暂缓开展：生产 command consumer、通用多 route/多 transaction scheduler 与 event bus、checkpoint、branch restore、active-run truncating reset、完整 RunResource hooks、完整 DiagnosticRecord/PolicyDecision/renderer/store/artifact 链、YAML/INI、多端 adapter、runtime registry、serializer、StateFragment，以及 R4 Field/Artifact/Dataset/sink 与 R4～R8 其余能力。
- 旧 GNCZMKN 只作为只读行为与科学参照，不进入任何生产 target、include path 或运行依赖。

阶段顺序已经固定：R1 以 Definition、PreparedModel 和 Kernel 的无 Session 独立求值闭合；R2 实现 MissionSource 到已证明、已链接 ExecutionPlan 的静态编译；R3 已完成 formal REF-YYZ run、Completed-run reset、plan-derived cancellation、显式 dispose、Session-local store/frame/IntegrationScope、窄 in-process outcome，以及首条 qualification command/event 原子事务，后续补齐生产调度 consumer、checkpoint/branch restore/RunResource 和完整运行诊断能力。
