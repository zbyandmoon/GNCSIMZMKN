...
这是 GNCZMKN 大型目标架构的 greenfield 实现仓库。仓库版本仍为 `0.0.0-bootstrap`；R0 科学基线、R1 独立模型生态与 R2 静态编译已经闭合，R3 已从 Image-backed Session 初始化推进到同一 Session 内两次 Continue 与一次 Terminal 的完整 REF-YYZ StepTransaction 纵向切片。
...
- 当前 R3 切片只消费冻结 Image 与显式 package/generated adapter，已覆盖结构预检、typed materializer identity、`Created → Initialized`、3 个 prepared artifact、7 个 Session-local Runtime Cell、2 个 committed/candidate state object、Image storage extent 分配、强异常安全初始化，以及 Session-local `CommittedStateStore`、`TransactionCandidateStore`、`SessionRuntimeBindings`、bounded `CycleFrame`、最小 `StepJournal`、Image-backed committed history 和最新 sealed boundary storage。Runtime Cell factory 只能取得其 Image 精确声明且去重后的 preparation 依赖；state、frame、candidate 与 evaluator history 访问均由调用范围和 owner 精确授权。Kernel 不读取 source、不查询 Catalog，也不包含 YYZ 具体类型分支。
- Session 通过统一 `execute_step()` 从 Image `TransactionPlan` 选择分支：两次 Continue 用固定步长 RK4、transaction-local FrozenInterval held form 与质量区间演化把 `(epoch=0,tick=0)` 推进到 `(1,1)` 与 `(2,2)`，第三步在 tick 2 执行完整 boundary DAG 和真实 committed-history evaluator，再以 Terminal instant ModelCommit 推进到 `(3,2)`。Terminal 保持刚体与质量状态字节不变，把两个 state block epoch 同步推进到 3，先 seal tick 2 observation，再 seal 与 `ORACLE-YYZ-MISSION-COMPOSITION-001` 对齐的 mission result；三份 chronological rigid/mass history 均由 Session 深拷贝持有。
- Continue 成功同步提交 rigid/mass candidate、当前 boundary history 与 ObservationSeal；held form 在提交和 seal 后随 frame 销毁。任一执行期提交前故障都会保留上一次 committed state、history、seal、epoch 与 tick，清理当前 staging，并令 Session 进入 `Failed`；该 Session 拒绝继续执行，独立新 Session 仍可正常完成。Terminal 成功后 Session 进入 `Completed` 并拒绝额外 step。
- 暂缓开展：完整周期 scheduler、event/cancellation、CommandLedger/queues、通用多 transaction/多 integration scope 执行、reset/checkpoint/restore、完整 RunId/RunBinding 与 Diagnostic/PolicyDecision/RunOutcome、YAML/INI、多端 adapter、runtime registry、serializer、StateFragment，以及 R4 Field/Artifact/Dataset/sink 与 R4～R8 其余能力。
- 旧 GNCZMKN 只作为只读行为与科学参照，不进入任何生产 target、include path 或运行依赖。

阶段顺序已经固定：R1 以 Definition、PreparedModel 和 Kernel 的无 Session 独立求值闭合；R2 实现 MissionSource 到已证明、已链接 ExecutionPlan 的静态编译；R3 已完成正式 Session 初始化、opening boundary 和首个 terminal-capable REF-YYZ StepTransaction 切片，后续补齐通用调度、生命周期与运行诊断能力。
