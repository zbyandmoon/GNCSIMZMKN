...
这是 GNCZMKN 大型目标架构的 greenfield 实现仓库。仓库版本仍为 `0.0.0-bootstrap`；R0 科学基线、R1 独立模型生态与 R2 静态编译已经闭合，R3 已形成 caller-bound InitializationCommit、两次 Continue、一次 Terminal、Completed-run ResetCommit、plan-derived cancellation、稳定历史 RunOutcome、显式 dispose，以及 route-free REF-YYZ process-local checkpoint/branch restore 串联的 formal run，并补充 qualification-only ModeOwner command/event 原子事务和真实 YYZ guidance→controller 双速率 `HeldLatest` 切片。
...
- R2 Image 保存 exact package/build lock、按 storage class 划分的确定性 extent、slot size/alignment/offset/codec/writer/reader、两个 state codec、Query `CallerLocal` route、Closure `HeldInterval` route、fixed-step RK4 policy、完整 `TransactionPlan` 分支语义、reset lifecycle capability、plan-derived cancellation safe points、preparation lifecycle，以及七个 typed `RuntimeCellFactory` 的数字依赖闭包。R3 additive lowering 还可冻结显式 `HeldLatest` binding 的 committed-output slot、producer/coordinator writer、reader set 与最大 age；这些事实进入既有 proof、descriptor hash 和 Image fingerprint，无 held edge 的 interval-1 Image 编码保持原值。
...
- 独立 qualification profile 沿普通 product definition→Compiler→link→Session 链把 package-owned guidance 设为 interval 2/offset 0，并让 controller 每 tick 执行。tick 0 与 tick 2 使用 fresh guidance，tick 1 从 Session-local `CommittedOutputStore` 注入保留原 sequence/timing/quality 的 tick 0 样本，controller 读取到 age 1 并生成可核对的正式 moment command。该 2:1 profile 只验证框架语义，完整 YYZ guidance 10 Hz/20 Hz 与 30 秒 Image 仍等待 owner 决定。
...
YYZ package 还贡献 `AltitudePitchGuidance` 的 stateless `SampledTransform` descriptor：`vehicle.process` placement、`process` phase、`BoundaryEvaluation`、zero-order hold、`Instantiate | Resettable | Dispose` lifecycle 和 exact guidance kernel identity。默认 `ReferenceInterval1` profile 保持每个 committed boundary 执行及 current-cycle sampled ports；独立 `MultirateHeldOutputQualification` profile 只把真实 guidance→controller edge 改为显式 `HeldLatest`、guidance interval 2/offset 0 与 controller age 1。两者均不裁决 00A/Reference A 的完整目标频率。canonical config 可确定性重建 definition，Catalog 对 form/profile/recipe/obligation/schedule/port/lifecycle 组合做封闭校验：
