...
当前产品已具备 R1 PureQuery/Closure/AlgorithmKernel、R2 proven/linked `ExecutionPlanImage` 与 R3 Image-backed Session/StepTransaction；仓库所有者已经判定 G3 `Passed`。package-owned typed RuntimeCellFactory、state/value codec、caller-local/held result route、确定性 storage extent、唯一 writer/reader、fixed-step RK4 policy、transaction branches、preparation lifecycle 与 factory direct handles 已进入 Descriptor/Image。R3 进一步执行 committed/candidate/frame/history/seal stores、integration、multi-owner commit、command/event、held outputs、checkpoint/restore、scheduled fault、two-entity selector/scope union 与 predeclared inactive-child activation。下表保留 R0-ARCH-002 当时的分阶段启用计划，不作为当前 gate 状态镜像；当前 R3 preflight 以 `docs/handoff/release-gates.md` 为准，完整 G6 deletion guard 仍为 `deferred-by-gate`。
