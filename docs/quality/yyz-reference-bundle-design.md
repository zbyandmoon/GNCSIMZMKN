...
R3 canonical 记录由 `r3.kernel-yyz-00a-canonical.probe` 与 `r3.yyz-00a-canonical.oracle` 形成。source 选择 `aero-table.fixture.yyz.00a-synthetic-multiaffine@2`，canonical Image fingerprint 为 `65e00515234efcabfae14399e9fe6838d65248af2d9a598f50b09a3470131078`；此前记录的 `4faccb3be9c6b760591b286c8996322069c7e0c1ba1668dcdf8946fb36c263ff` 是文档漂移，未改变 accepted source、asset 或 interval-1 identity。冻结的 `aero-table.fixture.yyz.multiaffine@1` 在 opening Mach `0.6176470588235294` 继续返回 `OutOfRange`；successor 保留旧八角点、旧域查询结果与通用严格三线性 kernel。初始 successor 在真实 tick 9 暴露 alpha `0.113627` 的有限域失败，唯一一次授权调整把域固定为 Mach `[0.2,0.8]`、alpha `[-pi,pi]`、beta `[-pi/2,pi/2]`。每个 qualification build 中的两次真实 Session 均从 `[0,0,1000] m`、`[220,0,0] m/s`、`680 kg` 与 level-east passive attitude 运行到 `Completed / duration-complete`，提交 tick 3000 和 3000 个 intervals，并在同一 build/process 内按位确定。独立 80 位 Decimal 路径验证 successor 生成、27 个旧域查询及 opening lookup，不复制 3000-step Session/RK4。revision-3 difference report 保留一份完整 build-local capture；跨 MSVC/GCC 的冻结 projection 只比较可移植的 identity/status/cadence/held/terminal、opening tolerance、terminal mass 与有限有序域内查询包线，不声明长时程 state 跨工具链位级相等。最大可移植绝对差为 terminal mass 上的 `1.364e-11`，位于组合容差内；`unresolved_count=0`，verdict 为 `abstract_engineering_target_conformance`。
...
当前 `reports/r3-yyz-001-00a-difference.json` revision 3 采用 exact identity/status、逐字段 opening 与 mass abs/rel `2e-12`，并对长时程 build-local observation 派生有限性、shape、顺序、selected-domain、opening containment 与同 build/process 双运行按位确定性。原始 terminal state 和实际气动查询包线保留在代表性 capture 中，但不冒充跨 compiler/runtime/libm 的 exact trajectory oracle。18 项独立数值比较与 21 项 exact/invariant 比较均 accepted；最大可移植绝对差为 terminal mass 上的 `1.364e-11`，低于组合阈值 `1.33200000000002728e-9`。`unexplained_difference_count=0`、`unresolved_count=0`，`candidate_terminal_science_verdict=true`，verdict 为 `abstract_engineering_target_conformance`。
...
- 状态审查：仓库所有者已将 R3-YYZ-001 置为 `done`；R3-FIX-001 与 R3-DIA-001 的 technical acceptance evidence 已闭合并保持 `review`、`assignee:null`，等待 owner acceptance。G4/G5/G6 尚无 owner decision，R4+ 保持锁定。
