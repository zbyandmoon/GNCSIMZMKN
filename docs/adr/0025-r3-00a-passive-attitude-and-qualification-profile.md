...
10. The canonical Image fingerprint is `4faccb3be9c6b760591b286c8996322069c7e0c1ba1668dcdf8946fb36c263ff`. The prior domain-locked canonical fingerprint was `e981118b136b3e6872da40a00c296153b9ad26e144c7882341b64b30b219574c`; the interval-1 fingerprint remains `7d1fbe1fa09ca555420ed3cc14a05d1f2994c6501b17cd3991355520fc8e6f14`.
...
12. Two real canonical Sessions reach tick 3000 with `3000` committed intervals and a `Completed / duration-complete` terminal result. Cadence counts are navigation `3001`, guidance `601`, controller `1501` and actuator `3001`; maximum `HeldLatest` ages remain `4/1`, and nonzero actuator output reaches an RK4 held form. The actual query envelope is Mach `[0.30237994030302495,0.61764705882352944]`, alpha `[-3.1412239572488261,3.1397707600310452]` and beta `[-1.4925695311254403,1.4441212960610592]`. State, aggregate, terminal window, RunOutcome, envelope and cadence are bit-deterministic across both runs. The compact difference report has `unresolved_count == 0` and emits `abstract_engineering_target_conformance` within the declared abstract-engineering claim. Real-aircraft accuracy, stability, handling quality and certification remain outside that claim.
...
- `r3.yyz-00a-canonical.oracle` runs the independent frozen opening reference and validates `reports/r3-yyz-001-00a-difference.json`. All compared fields pass; maximum absolute error is `1.364e-11` on terminal mass within the combined tolerance, and `unresolved_count` is 0.
